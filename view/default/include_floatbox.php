...
$(function(){
	...
	var post_award = function(){
		$.post("/torrents/<?=$data['current_torrent']['id']?>/award/",{count:$("#count").val(),inajax:1},function(data){
			ui.notify('提示', data.msg).effect('slide');
			$(".close-this").click();
		},"json")
	}
	var get_award = function(){
		$.getJSON("/torrents/<?=$data['current_torrent']['id']?>/award_cloud/",function(data){
