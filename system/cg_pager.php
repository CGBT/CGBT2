...
.inactivePrev, .inactiveNext{
	...
	public function paginate($page, $itemPerPage = 0)
	{
		...
		if ($this->_noNextPrev)
		{
			$page_url = str_replace('$page', $prev_page, $this->baseUrl);
			$this->components['prev_link'] = ($this->currentPage != 1 && $this->totalItem >= $this->maxLength) ? "<a class=\"{$this->prevCss}\" href=\"{$page_url}\">{$this->prevText}</a> " : "<span class=\"{$this->inactivePrevCss}\">{$this->prevText}</span> ";
		}
		else
		{
			$page_url = str_replace('$page', $prev_page, $this->baseUrl);
			$this->output = ($this->currentPage != 1 && $this->totalItem >= $this->maxLength) ? "<a class=\"{$this->prevCss}\" href=\"{$page_url}\">{$this->prevText}</a> " : "<span class=\"{$this->inactivePrevCss}\">{$this->prevText}</span> ";
		}
		...
		if ($this->totalPage > $this->maxLength)
		{
			...
			for($i = 1; $i <= $this->totalPage; $i++)
			{
				...
				if ($i == 1 || $i == $this->totalPage || in_array($i, $range))
				{
					...
					$page_url = str_replace('$page', $i, $this->baseUrl);
					$this->output .= ($i == $this->currentPage) ? "<a class =\"{$this->currentCss}\" href=\"javascript:void(0);\">$i" : "<a class=\"{$this->pagesCss}\"  href=\"{$page_url}\">$lastDot $i";
			...
			$page_url = str_replace('$page', $next_page, $this->baseUrl);
			if ($this->_noNextPrev)
			{
				$this->components['next_link'] = ($this->currentPage != $this->totalPage && $this->totalItem >= $this->maxLength) ? "<a class=\"{$this->nextCss}\" href=\"{$page_url}\">{$this->nextText}</a>\n" : "<span class=\"{$this->inactiveNextCss}\">{$this->nextText}</span>\n";
			}
			else
			{
				$this->output .= ($this->currentPage != $this->totalPage && $this->totalItem >= $this->maxLength) ? "<a class=\"{$this->nextCss}\" href=\"{$page_url}\">{$this->nextText}</a>\n" : "<span class=\"{$this->inactiveNextCss}\">{$this->nextText}</span>\n";
			}
		}
		else
		{
			...
			{
				$page_url = str_replace('$page', $i, $this->baseUrl);
				$this->output .= ($i == $this->currentPage) ? "<a class=\"{$this->currentCss}\" href=\"javascript:void(0);\">$i</a> " : "<a class=\"{$this->pagesCss}\" href=\"{$page_url}\">$i</a> ";
			}
			$page_url = str_replace('$page', $next_page, $this->baseUrl);
			if ($this->_noNextPrev)
			{
