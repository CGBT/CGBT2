...
class cg_app
{
	...
	{
		if (strpos($_SERVER['REQUEST_URI'], "http://" . $_SERVER['HTTP_HOST']) === 0)
		{
			$_SERVER['REQUEST_URI'] = str_replace("http://" . $_SERVER['HTTP_HOST'], '', $_SERVER['REQUEST_URI']);
		}
		
		if (strpos($_SERVER['REQUEST_URI'], "https://" . $_SERVER['HTTP_HOST']) === 0)
		{
			$_SERVER['REQUEST_URI'] = str_replace("https://" . $_SERVER['HTTP_HOST'], '', $_SERVER['REQUEST_URI']);
		}
		cg::load_core('cg_router');
		$router = new cg_router();
		$routeRs = $router->execute($this->route, cg::config()->APP_SUBDIR);
		$moduleName = '';
		if ($routeRs[0] !== null && $routeRs[1] !== null)
		{
			//dispatch, call Controller class
			require_once cg::config()->SYS_PATH . "cg_controller.php";
			...
			if ($routeRs[0][0] !== '[')
			{
				if (strpos($routeRs[0], '\\') !== false)
				{
					...
					$nsClassFile = explode(cg::config()->APP_NAMESPACE_ID . '/', $nsClassFile, 2);
					$nsClassFile = $nsClassFile[1];
					require_once cg::config()->APP_PATH . $nsClassFile . '.php';
				}
				else
				{
					require_once cg::config()->APP_PATH . "controller/base_controller.php";
					require_once cg::config()->APP_PATH . "controller/{$routeRs[0]}.php";
				}
			}
			else
			{
				...
				if (isset(cg::config()->APP_PATH_ORI) === true)
				{
					require_once cg::config()->APP_PATH . cg::config()->APP_PATH_ORI . 'module/' . $moduleName . '/controller/' . $moduleParts[1] . '.php';
				}
				else
				{
					require_once cg::config()->APP_PATH . $moduleName . '/controller/' . $moduleParts[1] . '.php';
					cg::config()->APP_PATH_ORI = cg::config()->APP_PATH;
				}
				
				//set class name
				$routeRs[0] = $moduleParts[1];
				cg::config()->APP_PATH = cg::config()->APP_PATH_ORI . 'module/' . $moduleName . '/';
			}
			...
			if ($clsnameDefined)
			{
				if ($rs = $controller->beforeRun($routeRs[3], $routeRs[1], $moduleName))
				{
					return $rs;
				}
			}
			...
			{
				if ($rs = $controller->beforeRun($routeRs[0], $routeRs[1], $moduleName))
				{
					return $rs;
				}
			}
			$method_name = $routeRs[1] . '_action';
		...
		else if (cg::config()->AUTO_ROUTE)
		{
			list($controller_name, $method_name, $method_name_ori, $params, $moduleName) = $router->auto_connect(cg::config()->APP_SUBDIR, (isset($this->route['autoroute_alias']) ===
			 true) ? $this->route['autoroute_alias'] : null);
			...
			cg::config()->APP_PATH_ORI = cg::config()->APP_PATH;
			cg::config()->APP_PATH = cg::config()->APP_PATH_ORI . 'controller/' . $moduleName . '/';
			
			$controller_file = cg::config()->APP_PATH . "{$controller_name}.php";
			
			if (file_exists($controller_file))
			{
				require_once cg::config()->SYS_PATH . "cg_controller.php";
				require_once cg::config()->APP_PATH_ORI . "controller/base_controller.php";
				...
				if ($methodsArray === null && isset(cg::config()->APP_NAMESPACE_ID) === true)
				{
					if (isset($moduleName) === true)
					{
						$controller_name = cg::config()->APP_NAMESPACE_ID . '\\controller\\' . $moduleName . '\\controller\\' . $controller_name;
					}
					else
					{
						$controller_name = cg::config()->APP_NAMESPACE_ID . '\\controller\\' . $controller_name;
					}
					$methodsArray = get_class_methods($controller_name);
				}
				...
				{
					if (isset(cg::config()->APP_PATH_ORI) === true)
						cg::config()->APP_PATH = cg::config()->APP_PATH_ORI;
					$this->throwHeader(404);
					return;
				}
			}
			else if (isset($moduleName) === true && isset(cg::config()->APP_NAMESPACE_ID) === true)
			{
				if (isset(cg::config()->APP_PATH_ORI) === true)
					cg::config()->APP_PATH = cg::config()->APP_PATH_ORI;
				
				$controller_file = cg::config()->APP_PATH . '/controller/' . $moduleName . '/' . $controller_name . '.php';
				...
				$controller_name = cg::config()->APP_NAMESPACE_ID . '\\controller\\' . $moduleName . '\\' . $controller_name;
			...
			if ($_SERVER['REQUEST_METHOD'] === 'PUT')
				...
				//before run, normally used for ACL auth
			if ($rs = $controller->beforeRun($controller_name, $method_name, $moduleName))
			{
				return $rs;
			}
	...
	{
		if (cg::config()->APP_SUBDIR != '/')
			$_SERVER['REQUEST_URI'] = substr(cg::config()->APP_SUBDIR, 0, strlen(cg::config()->APP_SUBDIR) - 1) . $routeuri;
		else
	...
	public function module($moduleUri, $action = null, $params = null)
	{
		...
		{
			if (cg::config()->APP_SUBDIR != '/')
				$_SERVER['REQUEST_URI'] = substr(cg::config()->APP_SUBDIR, 0, strlen(cg::config()->APP_SUBDIR) - 1) . $moduleUri;
			else
				$_SERVER['REQUEST_URI'] = $moduleUri;
			
			$tmp = cg::config()->APP_PATH;
			if (isset(cg::config()->APP_PATH_ORI) === true)
			{
				cg::config()->APP_PATH = cg::config()->APP_PATH_ORI;
				$tmpOri = cg::config()->APP_PATH_ORI;
			}
			...
			cg::config()->APP_PATH = $tmp;
			
			if (isset($tmpOri) === true)
				cg::config()->APP_PATH_ORI = $tmpOri;
		...
		else if (is_string($moduleUri))
		{
			...
			require_once cg::config()->APP_PATH . cg::config()->APP_PATH . "controller/$moduleUri.php";
			$controller = new $controller_name();
			$controller->params = $params;
			...
			if ($controller->autorender === true)
			{
				cg::config()->AUTO_VIEW_RENDER_PATH = array(
					strtolower(substr($controller_name, 0, -10)),
		...
		{
			require_once cg::config()->APP_PATH . cg::config()->APP_PATH . "controller/{$moduleUri[0]}.php";
	 ...
	 *
	 * Module rendered using this method is located in APP_PATH/APP_PATH/module
	 *
	...
	public function getModule($moduleName, $moduleUri, $action = null, $params = null)
	{
		...
		{
			if (isset(cg::config()->APP_PATH_ORI) === false)
			{
				cg::config()->APP_PATH_ORI = $tmp = cg::config()->APP_PATH;
				cg::config()->APP_PATH = $tmp . 'module/' . $moduleName . '/';
		...
		if ($code != null)
		{
			if (is_int($code))
			{
				if ($code === 404)
				{
					...
					header('HTTP/1.1 404 Not Found');
					if (!empty(cg::config()->ERROR_404_DOCUMENT))
					{
						include cg::config()->APP_PATH . cg::config()->ERROR_404_DOCUMENT;
					}
					...
					elseif (!empty(cg::config()->ERROR_404_ROUTE))
					{
						$this->reroute(cg::config()->ERROR_404_ROUTE, true);
					}
					exit();
				}
			...
			elseif (is_array($code))
			{
				...
				elseif ($code[1] === 302)
				{
					DooUriRouter::redirect($code[0], true, $code[1], array(
